...
const LAYOUT_ASSETS_DIR = path.join(process.cwd(), 'public', 'webwow', 'layouts', 'assets');
 ...
 * and save them to /public/webwow/layouts/assets/.
...
async function downloadAndLocalizeAssets(
  ...
  for (const assetId of uniqueIds) {
    ...
    try {
      ...
      if (existingFilename) {
        localUrlMap[assetId] = `/webwow/layouts/assets/${existingFilename}`;
        continue;
      }
      ...
      localUrlMap[assetId] = `/webwow/layouts/assets/${finalFilename}`;
    } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (imageFile) {
      const layoutsDir = path.join(process.cwd(), 'public', 'webwow', 'layouts', 'previews');
    ...
    const finalImagePath = `/webwow/layouts/previews/${uniqueLayoutKey}${imageExtension}`;
