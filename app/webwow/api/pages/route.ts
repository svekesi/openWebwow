...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[GET /webwow/api/pages] Error:', error);
    console.error('[GET /webwow/api/pages] Error message:', error instanceof Error ? error.message : 'Unknown error');
    console.error('[GET /webwow/api/pages] Error stack:', error instanceof Error ? error.stack : 'No stack');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name) {
      console.error('[POST /webwow/api/pages] Validation failed: missing name');
      return noCache(
    ...
    if ((error_page !== null || is_index) && slug && slug.trim() !== '') {
      ...
      console.error(`[POST /webwow/api/pages] Validation failed: ${pageType.toLowerCase()} page has non-empty slug`);
      return noCache(
    ...
    if (!is_index && error_page === null && (!finalSlug || finalSlug.trim() === '')) {
      console.error('[POST /webwow/api/pages] Validation failed: non-index page missing slug');
      return noCache(
    ...
    console.error('[POST /webwow/api/pages] Error:', error);
    console.error('[POST /webwow/api/pages] Error message:', error instanceof Error ? error.message : 'Unknown');
    console.error('[POST /webwow/api/pages] Error stack:', error instanceof Error ? error.stack : 'No stack');
