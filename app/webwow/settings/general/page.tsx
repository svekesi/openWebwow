...
export default function GeneralSettingsPage() {
  ...
  // Initialize Webwow badge from store
  const storedWebwowBadge = getSettingByKey('webwow_badge') as boolean | null;
  const [webwowBadge, setWebwowBadge] = useState(storedWebwowBadge ?? true);
  ...
  const handleSitemapTabChange = useCallback((tab: string) => {
    const mode: SitemapMode = tab === 'webwow-sitemap'
      ? 'auto'
      : tab === 'custom-sitemap'
        ? 'custom'
        : 'none';
  ...
  const saveWebsiteSettings = useCallback(async () => {
    ...
    await saveSettings({
      webwow_badge: webwowBadge,
      timezone,
      favicon_asset_id: faviconAssetId || null,
      web_clip_asset_id: webClipAssetId || null,
    });
    setIsSavingWebsite(false);
  }, [saveSettings, webwowBadge, timezone, faviconAssetId, webClipAssetId]);
                ...
                <Field className="col-span-2">
                  ...
                  <Tabs value={activeSeoTab} onValueChange={handleSitemapTabChange}>
                    <TabsList className="w-full">
                      ...
                      <TabsTrigger value="webwow-sitemap">Webwow generated</TabsTrigger>
                    ...
                    <TabsContent value="webwow-sitemap" className="mt-4 space-y-6">
                      <p className="text-sm text-muted-foreground">
