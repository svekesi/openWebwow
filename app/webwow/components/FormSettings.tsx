...
export default function FormSettings({ layer, onLayerUpdate }: FormSettingsProps) {
  ...
  useEffect(() => {
    const checkSmtpSettings = async () => {
      try {
        const response = await fetch('/webwow/api/settings/email');
        if (response.ok) {
    ...
    >
      {!isSmtpEnabled && isSmtpEnabled !== null && (
        <div className="text-xs text-muted-foreground text-center py-4">
          Enable <a href="/webwow/settings/email" className="underline hover:text-foreground">SMTP in Settings</a> to use email notifications.
        </div>
      )}
