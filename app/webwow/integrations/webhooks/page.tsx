...
export default function WebhooksPage() {
  ...
  const fetchWebhooks = async () => {
    try {
      const response = await fetch('/webwow/api/webhooks');
  ...
  const handleSaveWebhook = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `/webwow/api/webhooks/${editingWebhook.id}`
        : '/webwow/api/webhooks';
  ...
  const handleDeleteWebhook = async () => {
    ...
    try {
      const response = await fetch(`/webwow/api/webhooks/${webhookToDelete.id}`, {
        method: 'DELETE',
      });
  ...
  const handleToggleEnabled = async (webhook: Webhook) => {
    try {
      const response = await fetch(`/webwow/api/webhooks/${webhook.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleViewDeliveries = async (webhook: Webhook) => {
    ...
    try {
      const response = await fetch(`/webwow/api/webhooks/${webhook.id}/deliveries?limit=20`);
      const result = await response.json();
  ...
  const loadFilterData = async () => {
    ...
    try {
      const [formsRes, collectionsRes] = await Promise.all([
        fetch('/webwow/api/form-submissions?summary=true'),
        fetch('/webwow/api/collections'),
      ]);
