...
export default function AppsPage() {
  ...
  const fetchApps = async () => {
    try {
      const response = await fetch('/webwow/api/apps');
  ...
  const loadMailerLiteSettings = async () => {
    ...
    try {
      const response = await fetch('/webwow/api/apps/mailerlite/settings');
      const result = await response.json();
  ...
  const handleTestApiKey = async () => {
    ...
    setIsTesting(true);
    try {
      const response = await fetch('/webwow/api/apps/mailerlite/test', {
        method: 'POST',
  ...
  const handleDisconnect = async () => {
    try {
      await fetch('/webwow/api/apps/mailerlite/settings', {
        method: 'DELETE',
      });
  ...
  const loadGroupsAndForms = useCallback(async () => {
    ...
    try {
      const [groupsRes, formsRes] = await Promise.all([
        fetch('/webwow/api/apps/mailerlite/groups'),
