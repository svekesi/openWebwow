...
exports.findNearestCssRule = findNearestCssRule;
exports.buildCssDeclarationEdit = buildCssDeclarationEdit;
function findNearestCssRule(text, cursorOffset) {
...
function buildCssDeclarationEdit(text, match, property, value) {
...
function parseDeclarations(text, blockStart, blockEnd) {
    const declarations = [];
...
function detectLineIndent(text, index) {
...
function needsLeadingLineBreak(text, blockStart, blockEnd) {
    const content = text.slice(blockStart + 1, blockEnd).trim();
    return content.length > 0;
}
function needsTrailingLineBreak(text, blockEnd) {
